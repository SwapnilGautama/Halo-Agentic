...
def init_system():
    ...
    # LOAD FIELD DIRECTORY (Semantic Translation)
    field_rules = ""
    ...
    if os.path.exists("ut_data.xlsx"):
        ...
        # Ensure 'Month' (Name) is unique for display, and 'Month' (Number) is for sorting
        cols = list(df_ut.columns)
        # Based on your file: Col 2 is Num, Col 25 is Name
        cols[1] = "Month_Num" 
        cols[-1] = "Month_Label"
        df_ut.columns = cols
...
with st.sidebar:
    ...
    with st.expander("View Field Mappings"):
        st.text(field_definitions)
    with st.expander("View KPI Formulae"):
        st.write(kpi_map)
...
# 3. EXECUTION ENGINE
if prompt := st.chat_input("Ask: What is the FTE trend by Customer?"):
    ...
    with st.chat_message("assistant"):
        ...
        # THE ARCHITECT: The master of translation
        # 
        architect_msg = f"""
        ...
        FIELD DEFINITIONS (Use these to find column names for user terms):
        {field_definitions}
        
        RULES:
        1. For any TREND, use 'Month_Label' as the X-axis and 'Month_Num' for sorting.
        2. Always include 'ORDER BY Month_Num ASC' if 'Month_Num' is in the query.
        3. Use 'ut_data' for utilization/fte/hc. Use 'pnl_data' for financial values.
        4. Use double quotes for all column names: e.g., "FinalCustomerName".
        ...
        try:
            ...
            # CFO Narrative
            narrative = llm.invoke(f"Summarize these findings for a CFO: {df.to_string()}").content
            st.markdown(narrative)
            st.dataframe(df)
            
            # Intelligent Charting
            if not df.empty and len(df.columns) >= 2:
                # Decide chart type based on presence of Month
                chart_type = px.line if "Month" in str(df.columns) else px.bar
                fig = chart_type(df, x=df.columns[0], y=df.columns[1], markers=True, title=f"Analysis: {prompt}")
